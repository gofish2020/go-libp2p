...
func makeBasicHost(listenPort int, insecure bool, randseed int64) (host.Host, error) {
	...
	priv, _, err := crypto.GenerateKeyPairWithReader(crypto.RSA, 2048, r) // rsa 生成 私钥
	if err != nil {
		return nil, err
	}
	...
	opts := []libp2p.Option{
		...
		// /ip4/127.0.0.1/tcp/0     port = 0 表示随机端口  port =2222 表示固定端口 2222
...
func startListener(ctx context.Context, ha host.Host, listenPort int, insecure bool) {
	...
	ha.SetStreamHandler("/echo/1.0.0", func(s network.Stream) { // 本质就是  yamux.Stream
...
func runSender(ctx context.Context, ha host.Host, targetPeer string) {
	...
	maddr, err := ma.NewMultiaddr(targetPeer) // targetPeer =  /ip4/127.0.0.1/tcp/2222/p2p/QmcX4KsBCoqyAfQ1hTVSgAB3RxMNaeRSZf3pAGAcxrKTVc
	if err != nil {
		log.Println(err)
		return
	}
	...
	ha.Peerstore().AddAddrs(info.ID, info.Addrs, peerstore.PermanentAddrTTL) // 保存 id 和 地址关系
